...
- 支持敏感动作确认、最大步数限制、停止运行，以及高级区中的上下文重置和运行日志导出。
...
6. 前端解析、校验动作，并自动执行安全动作；敏感动作仍会请求确认。
7. 重复执行，直到模型返回 `done`、请求 `take_over`、达到最大步数或用户停止。
...
src/
  ...
  components/
    ...
    RunLog.tsx                # 运行日志
    RunPanel.tsx              # 聊天、高级调试入口和待执行动作
    RunControls.tsx           # 高级调试和停止控制
